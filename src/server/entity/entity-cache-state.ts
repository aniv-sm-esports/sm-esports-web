...
export class EntityCacheState<T extends IRepositoryEntity<T>> {
  ...
  protected filter:EntityCacheSearch<T>;
  ...
  public static from<T extends IRepositoryEntity<T>>(data:EntityCacheStateData<T>) {
    return new EntityCacheState<T>(
      ...
      new EntityCacheSearch<T>(data.filterSearch, data.filterSearchSettings));
  }
  ...
  //
  set(parentState:EntityCacheState<T>) {
    ...
    let changed =  EntityCacheStateDiffer.filterDiff(this.filter, parentState.filter);
